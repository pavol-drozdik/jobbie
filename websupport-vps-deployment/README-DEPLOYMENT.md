...
4. **Optional variable** `STAGING_HEALTH_URL` — when unset, staging deploy workflows derive `https://{APP_DOMAIN}/health` from the VPS `.env` before calling `deploy_backend.sh` (fails the job if `APP_DOMAIN` is also missing).
