      ...
      <div class="mt-[30px] flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div class="min-w-0">
          <h1 class="m-0 font-dmSans text-[28px] font-extrabold leading-tight text-black sm:text-[32px]">
            {{ S.foreignJobHubTitle }}
          </h1>
          <p class="m-0 mt-2 max-w-2xl text-[15px] leading-relaxed text-black/50">
            {{ S.foreignJobHubPageDescription }}
          </p>
        </div>
        <NuxtLink
          :to="ROUTES.foreignJobNew"
        ...
        >
          {{ jobs.length ? S.foreignJobHubNewAnother : S.foreignJobHubNewJob }}
        </NuxtLink>
      </div>
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-4">
              ...
              <div class="min-w-0 text-center sm:text-left">
                <p class="m-0 text-[17px] font-bold leading-snug text-black">
                  {{ S.foreignJobHubEmptyCalloutTitle }}
                </p>
                <p class="m-0 mt-2 text-[15px] leading-relaxed text-black/50">
                  {{ S.foreignJobHubEmptyCalloutBody }}
                </p>
                <p class="m-0 mt-4">
                  <NuxtLink
                    :to="ROUTES.foreignJobNew"
                  ...
                  >
                    {{ S.foreignJobHubEmptyCalloutLink }}
                  </NuxtLink>
                </p>
              </div>
            </div>
...
const { api } = useApi()
const { jobs, loading, loadError, loadJobs } = useProfileMyJobs({ isForeign: true })
...
function openRename(job: Job): void {
  ...
  renameTitle.value = job.title?.trim() || S.foreignJobHubDefaultTitle
...
onMounted(async () => {
  ...
  if (typeof q === 'string' && q.length > 0) {
    await router.replace({ path: ROUTES.foreignJobWizard(q) })
    return
  }
