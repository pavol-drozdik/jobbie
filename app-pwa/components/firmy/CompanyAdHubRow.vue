<template>
  ...
  >
    <div class="flex min-w-0 flex-1 items-stretch gap-3">
      <CategoryHubGlyph :category="ad.category" />
    ...
    <div class="flex w-full flex-col gap-2 pl-4 sm:w-auto sm:shrink-0 sm:flex-row sm:items-center sm:justify-end sm:pl-0">
      ...
      >
        ...
        >
          <span class="sr-only">{{ S.firmyHubMoreOptions }}</span>
...
import { getCategoryLabel } from '~/utils/job'
import type { CompanyAdListItem } from '~/utils/company-ad'
import { companyAdStatusLabel, isCompanyAdActive } from '~/utils/company-ad-hub'
...
const editHref = computed(() => ROUTES.myAdWizard(props.ad.id))

const previewHref = computed(() => ROUTES.professionalDetail(props.ad.id))

const canPreview = computed(() => isCompanyAdActive(props.ad))

const displayTitle = computed(
  () => props.ad.title?.trim() || S.firmyHubDefaultTitle,
)

const metaLine = computed(() => {
  const parts = [getCategoryLabel(props.ad.category), companyAdStatusLabel(props.ad)]
