...
import type { SkSchoolLevel, SkSchoolRow } from '~/utils/sk-school'
...
const { results, loading, scheduleSearch, dispose } = useSkSchoolSearch(() => props.level)
...
const searchPlaceholder = computed(() =>
  props.level === 'secondary' ? S.skSchoolSecondarySearchPh : S.skSchoolUniversitySearchPh,
)
...
function schoolSubtitle(row: SkSchoolRow): string {
  const parts: string[] = []
  if (row.municipality?.trim()) parts.push(row.municipality.trim())
  if (row.country === 'CZ') parts.push(S.skSchoolCountryCz)
  return parts.join(' · ')
}
...
function pickRow(row: SkSchoolRow): void {
  ...
  <div ref="rootRef" class="relative" :class="disabled ? 'cv-field-is-disabled' : ''">
    ...
    >
      ...
      <div class="app-form-dropdown__panel-list">
        ...
        >
          {{ row.name }}<span v-if="schoolSubtitle(row)" class="text-black/45"> · {{ schoolSubtitle(row) }}</span>
        </button>
        <button
          v-if="showUseCustom"
          type="button"
        ...
        >
          {{ S.skSchoolUseCustom.replace('{name}', trimmedQuery) }}
        </button>
      </div>
