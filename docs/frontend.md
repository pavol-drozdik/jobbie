...
- **SEO UI components:** [`components/seo/`](../app-pwa/components/seo/) (`AppBreadcrumbs`, `PublicContentAeoSummary`) — registered with `pathPrefix: false` in `nuxt.config.ts` (same pattern as `components/marketing/`).
...
- Font Awesome (register wizard, CV shell icons): `@fortawesome/fontawesome-free` via `assets/css/font-awesome.css` (CSP-safe; no CDN).
...
| `middleware/` | Client route guards (`auth`, `company-only`, …) — **not** API authorization |
...
| Role dashboards | `pages/dashboard/zakaznik.vue`, `pages/dashboard/poskytovatel.vue`, `components/dashboard/*`, `components/JobStatsPanel.vue`, `utils/dashboard-chart-theme.ts`, `utils/dashboard-display.ts`, `composables/useDashboardPeriod.ts` |
...
**Dashboards (`/dashboard/zakaznik`, `/dashboard/poskytovatel`):** white card shell on `bg-marketing-mint` (profile/settings pattern): back link, `JaSegmentedToggle` period presets, `DashboardMetricCard` tiles on `marketing-surface`, Chart.js via shared `dashboard-chart-theme.ts` (DM Sans, marketing green). Per-job stats on `/app/jobs/[id]` reuse the same components in `JobStatsPanel`.
...
**Home:** marketing landing at `/` (`pages/index.vue`) — A/B hero via [`HomeHeroSection.vue`](../app-pwa/components/home/HomeHeroSection.vue) (matches `Jobbie design/A_B _ Desktop.html`: dual-field search, role cards, trust row; phone column `public/img/jobbie-mobile-mockup.webp` from `Jobbie design/`); rollback: [`HomeHeroSection.legacy.vue`](../app-pwa/components/home/HomeHeroSection.legacy.vue). **Profile hub:** `/profil` — sidebar tabs: Môj profil, Uložené (`SavedPanel`), Predplatné (status + cancel via `SubscriptionStatusPanel`, upgrade link to `/cennik` on free plan), Recenzie (`ReviewsPanel`, read-only received reviews), Nastavenia účtu; provider stats link goes to `/dashboard/poskytovatel`. Legacy `?tab=provider` → dashboard, `?tab=buy-credits` → `/cennik`. Full public profiles at `/profil/[userId]` include `ProfileReviewForm` (POST/PATCH/DELETE `/api/profiles/:id/reviews`; `GET /api/profiles/:id` returns `viewer_review` for authenticated visitors). Sidebar **Napísať správu** calls `POST /api/profiles/:id/open-chat` and navigates to `/chat/:roomId` (or shows an application picker when several job threads exist); respects `public_allow_platform_contact`.

**Checkout (`/platba`):** auth-style split layout (`AuthMarketingSplitShell`, `rounded-[24px]` clipped card like login). Query: `type=credits&pack=<slug>` or `type=subscription&plan_id=<uuid>`. `StripePaymentForm` (`locale: sk`, `collectBusinessBilling`, deferred amount): billing fields use canonical `form-field-ui` classes; Stripe **Payment Element** + **Tax ID Element** on one screen with a single **Zaplatiť** (card, Google Pay, Apple Pay when enabled). Settings invoice pay and default payment method use the same Stripe appearance helper. Entry from `/cennik`, `/app/buy-credits`, profile buy-credits tab, and plan grids; optional `return` query (default `/cennik`).

**Pricing (`/cennik`):** public page (`pages/cennik.vue`), `max-w-[1400px]`, green gradient hero with tab toggle (kredity | plány | doplnkové služby); credit packs as a single white list panel; plans as one four-column white panel + `PricingCvCompareTable` (monthly credits, active offers, CV limits, and plan-tier publish/top costs from `GET /api/billing/config` → `planTierCreditCosts`); **Doplnkové služby** tab only (`PricingAddonServicesGrid` + split `PricingContactSection` → `POST /api/pricing-inquiries`). Job post wizard uses the same tier matrix for publish cost. Deep links: `?tab=credits` | `?tab=plans` | `?tab=addons`.
