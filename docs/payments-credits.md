...
6. Create finalized Stripe Invoice and mark **paid** (`attachPayment` / `paid_out_of_band`) with SK custom fields; `ensureCreditPaymentInvoice` then sends faktúra email via SMTP ([stripe-invoice-emails.md](./stripe-invoice-emails.md), [email-smtp.md](./email-smtp.md)). Stripe card receipt (`receipt_email`) is separate.
...
**`/platba` (primary):** `payment_behavior: 'default_incomplete'` per [Stripe’s recommended flow](https://docs.stripe.com/billing/subscriptions/overview#create-the-subscription) — subscription starts `incomplete`, Payment Element confirms the first invoice’s PaymentIntent (or `pending_setup_intent` during a trial), then status becomes `active` / `trialing` and Stripe sends recurring invoices automatically. On `subscriptions.create`, expand only `latest_invoice` and `pending_setup_intent` (Stripe allows max **4** expand levels from the root object — do not use `latest_invoice.payments.data.payment.payment_intent`). [`resolveSubscriptionPaymentClientSecret`](../backend-ts/src/payments/stripe.service.ts) follow-up: `invoices.retrieve` with payment expand, `confirmation_secret`, `paymentIntents.retrieve`, or `setupIntents.retrieve` when expanded objects omit `client_secret` (common on trial checkout).

**Free trial (Stripe-controlled):** on each subscription **Price** in Stripe, add metadata `trial_period_days` = `30` (or `jobbie_trial_days`). Avoid the Dashboard “trial on Price” catalog field (Stripe discourages it). To **disable**, remove that metadata. Optional dev fallback: `SUBSCRIPTION_TRIAL_FALLBACK_PERIOD_DAYS` when Prices have no trial yet. JOBBIE applies the trial only for **first-time** paid subscribers (`profiles.subscription_trial_used_at` + no meaningful Stripe subscription history — abandoned `canceled` incomplete checkouts without a paid invoice do not count). Catalog: `GET /api/billing/config` → `subscriptionTrial` + per-plan `trialPeriodDays`; `GET /api/plans` → `trial_period_days`; PWA `/cennik` shows badge when either field is &gt; 0. Checkout `/platba`: trial UI and deferred Elements mode are **frozen on page load** from `GET /api/payments/subscription-checkout-preview?plan_id=` (`resolveSubscriptionTrialDays` — same as create-intent); create-intent must not mutate that state mid-pay. Do not use catalog global trial fallback for checkout mount mode. Never `elements.update()` a PaymentIntent onto deferred-setup Elements — remount with `client_secret` and re-enter card instead (avoids Stripe `setup_future_usage: off_session` vs `null`). Credits checkout keeps deferred `payment` + `elements.update` for matching PaymentIntents.
...
In-app billing (Fakturácia): payment method via SetupIntent (`GET/POST /api/payments/payment-method*`), resume cancel (`POST /api/payments/resume-subscription`), invoice list (`GET /api/payments/invoices` — paid invoices only; open/unpaid invoices are not shown), invoice detail (`GET /api/payments/invoices/:invoiceId` — only accessible for paid invoices; returns 404 for open/unpaid). Seller block on detail: `invoice_supplier` from `GET /api/billing/config` (`BILLING_SUPPLIER_*` env). PWA: `/nastavenia/fakturacia/[invoiceId]` shows legal DPH footer. Plan changes: `/cennik` + `/platba`. [`payments.controller.ts`](../backend-ts/src/payments/payments.controller.ts).
