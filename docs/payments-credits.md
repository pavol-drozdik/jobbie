...
**Free trial (Stripe-controlled):** on each subscription **Price** in Stripe, add metadata `trial_period_days` = `30` (or `jobbie_trial_days`). Avoid the Dashboard “trial on Price” catalog field (Stripe discourages it). To **disable**, remove that metadata. Optional dev fallback: `SUBSCRIPTION_TRIAL_FALLBACK_PERIOD_DAYS` when Prices have no trial yet. JOBBIE applies the trial only for **first-time** paid subscribers (`profiles.subscription_trial_used_at` + no meaningful Stripe subscription history — abandoned `canceled` incomplete checkouts without a paid invoice do not count). Catalog: `GET /api/billing/config` → `subscriptionTrial` + per-plan `trialPeriodDays`; `GET /api/plans` → `trial_period_days`; PWA `/cennik` shows badge when either field is &gt; 0. Checkout `/platba`: the deferred Elements mode is driven by the **authoritative** `GET /api/plans` `trial_period_days` (derived from the live Stripe Price — the same value `createSubscriptionPaymentIntent` uses), gated by `subscriptionTrialEligible`; the checkout no longer falls back to catalog/global trial config for this decision (that caused `setup`-mode mounts while the server created a PaymentIntent → intent mismatch → stuck "Spracovávam…"). If a mismatch still occurs the form **remounts** Elements with the returned `client_secret` (`shouldRemountElementsForIntentMismatch`) and asks the user to re-enter the card and confirm again instead of submitting the empty remounted field; a ~30s watchdog around `confirmPayment`/`confirmSetup` guarantees the pay button never stays stuck.
