...
**PDF hlavička dodávateľa = Stripe Dashboard → Business details** (test aj live). API dopĺňa **IČO, DIČ, IČ DPH, OR a text oslobodenia od dane** do **pätičky** každej faktúry (`buildSkInvoiceFooter`). `BILLING_SUPPLIER_*` zrkadlí údaje aj v aplikácii `/nastavenia/fakturacia/:id`.
...
| IČO / DIČ | Pätička faktúry z API (`BILLING_SUPPLIER_*` / predvolené CoCreate) |
...
- `STRIPE_INVOICE_VAT_EXEMPTION_TEXT=` — § 74(h) text oslobodenia (predvolene: *Dodanie je oslobodené od dane. DPH: 0,00 €.*)
- `STRIPE_INVOICE_VAT_EXEMPTION_REFERENCE=` — voliteľný odkaz na zákon (doplňte po schválení účtovníkom)
- `STRIPE_INVOICE_FOOTER=` — voliteľné ďalšie riadky pätičky (za supplier + oslobodením)
...
- `BILLING_SUPPLIER_*` — IČO/DIČ/IČ DPH/OR na pätičke Stripe PDF a v aplikácii (predvolene CoCreate s. r. o.)
...
|------|------------|-------------------------|
| a | Dodávateľ — názov, sídlo, IČ DPH | Business details + Tax IDs (`eu_vat`); IČO/DIČ v pätičke z API |
| b | Odberateľ — názov, adresa, IČ DPH | Customer `name` + `address`; firma: `custom_fields` IČO, DIČ, IČ DPH |
| c | Poradové číslo | `invoice.number` (Dashboard numbering) |
| d | Dátum dodania / platby | Stripe `status_transitions.paid_at` |
...
| h | Oslobodenie od dane | Pätička: `STRIPE_INVOICE_VAT_EXEMPTION_TEXT` (+ voliteľný `…_REFERENCE`) |
| i | Výška dane | 0,00 € v predvolenom exemption texte |
| j–n | Špeciálne režimy | N/A pre SaaS |

**Obmedzenie Stripe:** IČO/DIČ/IČ DPH odberateľa (firma) sú v sekcii **custom fields**, nie v stĺpci „Na účet“.
...
- **Predplatné** — webhook `invoice.created` doplní popis riadku, pätičku (supplier IDs + oslobodenie), `custom_fields` odberateľa (z Customer `invoice_settings` alebo metadata) a obdobie predplatného na draft faktúre pred finalizáciou; otvorené faktúry majú `payment_settings: card`.
...
CoCreate má **IČ DPH** (identifikovaná osoba), ale **neúčtuje DPH na predaji** JOBBIE služieb. API pätička obsahuje predvolene *„Dodanie je oslobodené od dane. DPH: 0,00 €.“* — presný odkaz na zákon doplňte cez `STRIPE_INVOICE_VAT_EXEMPTION_REFERENCE` po schválení účtovníkom.
...
5. Skontrolujte: jeden riadok balík (nie 0,666 € × 30), hlavička dodávateľa z Dashboard, pätička s IČO/DIČ/IČ DPH dodávateľa a textom oslobodenia, odberateľ IČO/DIČ/IČ DPH (firma) v custom fields bez duplicitnej „SK VAT“, **žiadny** IBAN / Pay by Square / „Zaplatiť online“ na uhradenej kreditovej faktúre.
